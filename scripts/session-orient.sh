...
marker="absent"
[[ -f "$vault_abs/.arscontexta" ]] && marker="present"
...
next_action="Run arscontexta-health to establish a current baseline."
if [[ "$marker" == "absent" ]]; then
  next_action="Run arscontexta-setup if this directory should become an Ars Contexta vault."
...
printf 'Ars Contexta session orientation\n'
printf 'Vault: %s\n' "$vault_abs"
printf 'Marker: %s\n' "$marker"
printf '\n'
printf 'Inventory:\n'
