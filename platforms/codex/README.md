...
Ars Contexta uses explicit Codex skills and deterministic scripts for session
...
`arscontexta-session validate` is warning-only. It checks lightweight schema
signals after edits: frontmatter, `description:`, `topics:`, and obvious
unresolved wiki links.

`arscontexta-session capture` is explicit. It writes a markdown handoff into
...
add a design issue before wiring it into Ars Contexta.
