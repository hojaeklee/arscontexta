...
tmp_dir="$(mktemp -d "${TMPDIR:-/tmp}/arscontexta-health.XXXXXX")"
cleanup() {
  rm -rf "$tmp_dir"
}
trap cleanup EXIT
...
printf 'Ars Contexta Quick Health\n'
printf 'Overall: %s\n' "$overall"
printf 'Vault: %s\n' "$vault_abs"
printf 'Ars Contexta marker: %s\n' "$marker_status"
if [[ "$marker_status" == "absent" ]]; then
  printf 'Note: .arscontexta was not detected; treating this as a generic markdown vault.\n'
fi
printf '\n'
printf 'Inventory:\n'
