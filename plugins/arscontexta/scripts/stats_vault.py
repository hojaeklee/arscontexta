#!/usr/bin/env python3
"""Read-only Ars Contexta vault statistics."""
    ...
    try:
        ...
        with index.connect_readonly() as conn:
            ...
            if not schema_row or schema_row["value"] != SCHEMA_VERSION:
                raise StatsFallback("VaultIndex schema is stale; falling back to direct scan. Run arscontexta-index to rebuild.")
            ...
            if indexed_paths != set(current):
                raise StatsFallback("VaultIndex is stale; falling back to direct scan. Run arscontexta-index to rebuild.")
            for row in rows:
        ...
        raise StatsFallback("VaultIndex is missing; falling back to direct scan. Run arscontexta-index to build it.")
...
def interpretation_notes(metrics: dict[str, Any], vocab: dict[str, str]) -> list[str]:
    ...
    if metrics["orphans"] > 0:
        notes.append(f"{metrics['orphans']} orphan {vocab['note_plural']} -- run arscontexta-graph for details")
    if metrics["dangling"] > 0:
        notes.append(f"{metrics['dangling']} dangling links -- run arscontexta-graph to identify broken links")
    ...
    if metrics["large_vault_approximate"]:
        notes.append("Metrics approximate for large vault. Run arscontexta-graph for precise graph analysis.")
    return notes
...
def render_text(metrics: dict[str, Any], vocab: dict[str, str], queue: dict[str, Any], notes: list[str], limit: int) -> None:
    if metrics["total_files"] == 0:
        ...
        print("Topics: 0")
        print()
        print("Generated by Ars Contexta")
        return
