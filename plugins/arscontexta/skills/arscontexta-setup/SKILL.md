...
2. Detect context before writing:
   - Plugin repo: `.agents/plugins/marketplace.json` or `plugins/arscontexta/.codex-plugin/plugin.json` exists. Do not set up the repo as a vault.
   - Existing Ars Contexta vault: `.arscontexta` exists. Report what exists and fill missing minimal files only after explicit approval.
   - Existing markdown vault: markdown files or vault-like directories exist but `.arscontexta` is absent. Explain that setup will add scaffolding without moving notes.
