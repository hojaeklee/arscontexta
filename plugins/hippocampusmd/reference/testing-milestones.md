...
Seven validation milestones for the HippocampusMD v1.6 plugin. Each milestone tests a distinct layer of the system: from kernel correctness through vocabulary transformation to cross-platform parity. Run them in order — later milestones depend on earlier ones passing.
...
- `validate-kernel.sh` accessible at `plugins/hippocampusmd/reference/validate-kernel.sh`
...
plugins/hippocampusmd/reference/validate-kernel.sh /path/to/generated-vault
...
| Failure | Cause | Fix |
|---------|-------|-----|
| FAIL on primitive 1 (YAML frontmatter) | Template generation skipped frontmatter | Check `plugins/hippocampusmd/generators/features/templates.md` — ensure all templates output YAML blocks |
...
| Failure | Cause | Fix |
|---------|-------|-----|
| Missing "Common Pitfalls" section | Feature block conditional logic skipped it | Check `plugins/hippocampusmd/generators/features/ethical-guardrails.md` — pitfalls are always-on |
...
| Failure | Cause | Fix |
|---------|-------|-----|
| "MOC" appears in generated CLAUDE.md | Vocabulary transform missed a reference in a feature block | Search all feature block templates in `plugins/hippocampusmd/generators/features/` for hardcoded "MOC" |
...
for PRESET in "${PRESETS[@]}"; do
    ...
    plugins/hippocampusmd/reference/validate-kernel.sh "$VAULT"
    echo ""
    ...
    GRAN=$(grep 'granularity:' "plugins/hippocampusmd/presets/$PRESET/preset.yaml" | awk '{print $2}')
    PROC=$(grep 'processing:' "plugins/hippocampusmd/presets/$PRESET/preset.yaml" | awk '{print $2}')
    AUTO=$(grep 'automation:' "plugins/hippocampusmd/presets/$PRESET/preset.yaml" | awk '{print $2}')
    ...
    ACTIVE_BLOCKS=$(grep -A50 'active_blocks:' "plugins/hippocampusmd/presets/$PRESET/preset.yaml" | grep '^ *-' | sed 's/^ *- *//')
    ...
    if [ -f "$VAULT/self/identity.md" ]; then
        WARMTH=$(grep 'warmth:' "plugins/hippocampusmd/presets/$PRESET/preset.yaml" | awk '{print $2}')
...
echo "=== Milestone 1: Kernel ===" && plugins/hippocampusmd/reference/validate-kernel.sh /tmp/test-research
