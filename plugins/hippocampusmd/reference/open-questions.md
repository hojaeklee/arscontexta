...
**What:** A guided flow for importing existing note collections into a generated HippocampusMD system. Detects common formats (Obsidian, Notion export, plain markdown), maps existing structure to the three-space architecture, and proposes schema additions for existing frontmatter.
...
**When to ship:** When user feedback shows significant demand from existing vault owners who want to adopt HippocampusMD methodology on top of their current content.
...
**What:** A hosted version of the HippocampusMD MCP server that users connect to without running locally. Provides `hippocampusmd_query`, `hippocampusmd_recommend`, and `hippocampusmd_dimensions` tools through a cloud endpoint.

**Why deferred:** The local MCP server (`npx hippocampusmd-mcp`) works for v1. Hosting adds infrastructure, authentication, rate limiting, and operational costs. The trade-off is reach (easier onboarding) vs complexity (ops burden).
