...
1. Meta-documentation explaining the system's origin ("this system was derived from HippocampusMD methodology") may use the word "methodology" in its technical sense
...
5. **The kernel requires only filesystem access and conventions.** The minimal platform test (Test 5) proves that all 15 primitives can be satisfied through context file instructions alone. Automation improves reliability but is not required for structural completeness. This means HippocampusMD can generate valid systems for ANY platform that supports text files — the kernel's requirement floor is genuinely minimal.
