...
targets.each do |rel|
  ...
  if frontmatter.key?("description")
    ...
    if desc.empty?
      ...
      if desc.end_with?(".")
        add_finding(findings, "WARN", rel, "description", "Description ends with a period.", "Remove the trailing period to match HippocampusMD convention.")
      end
