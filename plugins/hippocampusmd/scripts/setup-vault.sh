...
write_file() {
  ...
  case "$rel" in
    .hippocampusmd)
      printf '%s\n' \
        '# HippocampusMD vault marker' \
        '# Generated by Codex setup.' \
        '# Do not delete unless this directory should stop being treated as a HippocampusMD vault.' \
        "created: $today" \
        'platform: codex' \
        > "$path"
      ;;
    ...
    ops/derivation.md)
      printf '%s\n' \
        ...
        "- Session workflow: hippocampusmd-session" \
        "" \
        "## Migration Notes" \
        "" \
        "Existing markdown files, if any, were left in place. This setup adds HippocampusMD scaffolding around the workspace without moving user content." \
        "" \
        "## Operating Model" \
        "" \
    ...
    ops/derivation-manifest.md)
      printf '%s\n' \
        "# ops/derivation-manifest.md -- Runtime configuration for HippocampusMD skills" \
    ...
    ops/config.yaml)
      {
        ...
        printf '%s\n' \
          ...
          "    - ops/queue/archive/**" \
          "automation:" \
          "  session_workflow: hippocampusmd-session" \
          "  explicit_user_confirmation: true" \
          "processing:" \
          "  mode: explicit" \
          "  pipeline_skill: hippocampusmd-pipeline" \
          "maintenance:" \
    ...
    manual/manual.md)
      printf '%s\n' \
        "---" \
        "description: User manual for this HippocampusMD vault" \
        "type: manual" \
        "created: $today" \
        "---" \
        "" \
    ...
    manual/skills.md)
      printf '%s\n' \
        ...
        "Available now:" \
        "" \
        "- \`hippocampusmd-help\` -- orient to the vault and choose the next action." \
        "- \`hippocampusmd-health\` -- run bounded read-only diagnostics." \
        "- \`hippocampusmd-setup\` -- create or complete minimal vault scaffolding." \
        "- \`hippocampusmd-session\` -- run explicit orient, validate, and capture workflows." \
        "" \
        "Use the installed HippocampusMD plugin for query, processing, maintenance, and evolution workflows." \
        > "$path"
      ;;
...
printf 'HippocampusMD minimal Codex setup\n'
printf 'Vault: %s\n' "$vault_abs"
printf 'Preset: %s\n' "$preset"
printf 'Domain: %s\n' "$domain"
if [[ "$dry_run" == true ]]; then
...
if [[ -f "$vault_abs/.agents/plugins/marketplace.json" || -f "$vault_abs/plugins/hippocampusmd/.codex-plugin/plugin.json" ]]; then
  printf 'ERROR: This looks like the HippocampusMD plugin repo, not a target vault. Choose another directory.\n' >&2
  exit 1
fi
...
printf '\n'
printf 'Next: run scripts/check-vault.sh "%s" from the HippocampusMD repo, then run hippocampusmd-health in Codex.\n' "$vault_abs"
