...
links_check() {
  ...
  local tmp_dir
  tmp_dir="$(mktemp -d "${TMPDIR:-/tmp}/hippocampusmd-mcp-links.XXXXXX")"
  ...
  printf '{\n'
  printf '  "tool": "hippocampusmd.links.check",\n'
...
frontmatter_validate() {
  ...
  vault_abs="$(resolve_vault "$vault")"
  local tmp_dir
  tmp_dir="$(mktemp -d "${TMPDIR:-/tmp}/hippocampusmd-mcp-frontmatter.XXXXXX")"
  ...
  printf '{\n'
  printf '  "tool": "hippocampusmd.frontmatter.validate",\n'
  printf '  "vault": "%s",\n' "$(printf '%s' "$vault_abs" | json_escape)"
  printf '  "overall": "%s",\n' "$overall"
