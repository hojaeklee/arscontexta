...
Since [[derivation generates knowledge systems from composable research claims not template customization]], the orchestrator doesn't generate the target vault from scratch. It uses HippocampusMD's init wizard to derive a properly configured knowledge system for the persona's domain, complete with domain-native vocabulary, appropriate processing pipeline, and coherent configuration. Orchestration adds the content layer on top of the structural layer that derivation provides.
