---
name: hippocampusmd-health
description: Use when the user asks Codex to check, diagnose, validate, or report on the health of a HippocampusMD or Obsidian markdown knowledge vault.
---

# HippocampusMD Health

Run a focused vault health check for a HippocampusMD markdown knowledge graph. Prefer concrete file counts, paths, and fixes over broad commentary.

## When Invoked

1. Treat the current working directory as the vault unless the user gives another path.
2. If the path contains `.hippocampusmd`, use it as a HippocampusMD vault.
3. If `.hippocampusmd` is absent, continue only when the directory still appears to be a markdown vault; state that HippocampusMD configuration was not detected.
4. If the user asks for `full` or `three-space`, load the shared reference at `plugins/hippocampusmd/reference/three-spaces.md` from the HippocampusMD plugin package when available. Otherwise keep the check quick.
...
```bash
plugins/hippocampusmd/scripts/vault-health.sh . --mode quick --limit 25 --format text
```

When running from an installed plugin package or a vault, discover the helper relative to the HippocampusMD repository or plugin root if needed:
...
- Vault marker and config: `.hippocampusmd`
...
- `WARN`: missing frontmatter fields, stale orphans, sparse links, pending operational queues, or absent optional HippocampusMD config
