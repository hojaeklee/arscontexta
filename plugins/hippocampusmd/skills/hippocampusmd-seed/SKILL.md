---
name: hippocampusmd-seed
description: Use when the user asks Codex to add a source file to a HippocampusMD processing queue, seed an inbox item, or prepare a source for later reduce, ralph, or pipeline processing.
---
...
4. Prefer the bundled helper:
   - `plugins/hippocampusmd/scripts/seed-vault.sh [vault-path] --file PATH --format text|json`
...
- next steps for `hippocampusmd-ralph` or `hippocampusmd-pipeline`
...
- Do not run `hippocampusmd-ralph`, `hippocampusmd-pipeline`, or downstream processing automatically.
