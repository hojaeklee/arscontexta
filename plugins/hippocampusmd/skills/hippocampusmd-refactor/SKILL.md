---
name: hippocampusmd-refactor
description: Use when the user asks Codex to pla HippocampusMD vault restructuring after config, derivation, architecture, schema, navigation, or automation changes.
---

# HippocampusMD Refactor
...
5. Recommend `hippocampusmd-architect` when no change rationale exists or the user is still deciding what should change.
...
- Consult `plugins/hippocampusmd/reference/interaction-constraints.md` for interaction constraints before recommending cascaded changes.
...
- If no config-vs-derivation changes are detected, report the clean state and suggest `hippocampusmd-architect` for exploratory evolution advice.
