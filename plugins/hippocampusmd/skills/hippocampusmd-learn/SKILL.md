---
name: hippocampusmd-learn
description: Use when the user asks Codex to research a topic, capture research into a HippocampusMD vault, or grow the knowledge graph from web results, local files, or provided source material.
---

# HippocampusMD Learn

Plan and capture research with provenance so the result can enter the HippocampusMD processing pipeline. Learn is an intake workflow: it gathers or organizes source material, writes an approved research capture into the inbox, and hands off to existing queue/pipeline skills.
...
10. Report the created file and recommend `hippocampusmd-seed` or `hippocampusmd-pipeline` for processing.
...
- `hippocampusmd-seed` for preparing the capture as a queued source.
- `hippocampusmd-pipeline` for planning visible end-to-end source processing.
...
- Do not mutate `ops/queue/*`; hand off to `hippocampusmd-seed` or `hippocampusmd-pipeline`.
