---
name: hippocampusmd-architect
description: Use when the user asks Codex to review or evolve an existing HippocampusMD vault architecture, diagnose system drift, or recommend evidence-backed vault changes.
---

# HippocampusMD Architect
...
- Compare current behavior against design intent and vocabulary rather than assuming universal HippocampusMD terms.
- Distinguish between urgent health failures, repeated friction, design drift, and speculative optimization.

## Research Grounding

- Consult local `plugins/hippocampusmd/reference/` sources first:
...
- Name whether the next step is `hippocampusmd-refactor`, `hippocampusmd-reseed`, or a separate explicit follow-up.
- Keep `hippocampusmd-architect` distinct from `hippocampusmd-recommend`: recommend advises new vault architecture; architect reviews and evolves an existing vault.
