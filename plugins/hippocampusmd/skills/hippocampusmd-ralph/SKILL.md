---
name: hippocampusmd-ralph
description: Use when the user asks Codex to inspect or process HippocampusMD queue tasks, run queued phases, dry-run queue work, or mark queue tasks advanced or blocked.
---

# HippocampusMD Ralph

Ralph is the standalone queue worker for HippocampusMD. Keep it separate from `hippocampusmd-pipeline`: Ralph executes explicit queue work, while `hippocampusmd-pipeline` will later decide when and how to orchestrate it.
...
- `hippocampusmd-seed` creates extract queue entries.
...
- completed batches that are ready for `hippocampusmd-archive-batch`

## Boundaries

- Ralph is not a background daemon.
- Ralph does not mutate queue state invisibly.
- Ralph does not replace `hippocampusmd-pipeline`; it is the explicit queue worker that pipeline may call later.
