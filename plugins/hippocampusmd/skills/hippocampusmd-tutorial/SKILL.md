---
name: hippocampusmd-tutorial
description: Use when the user asks Codex for a HippocampusMD tutorial, onboarding walkthrough, guided first use, or help learning the vault workflow by doing.
---

# HippocampusMD Tutorial

Guide a new user through HippocampusMD with a conversational, safe-by-default walkthrough. The tutorial teaches the shape of the system before it writes anything, and turns sample note creation into an explicit opt-in moment rather than a hidden side effect.
...
3. If those files are absent, use universal defaults: notes folder `notes/`, note name `note`, topic map `index`, and ordinary HippocampusMD markdown conventions.
...
- Label tutorial-created content clearly in frontmatter or body text, for example `tutorial: true` or a short sentence saying it was created during the HippocampusMD tutorial.
...
- After any sample note creation or link edit, recommend running `hippocampusmd-validate` on the changed notes.

## Step Guidance

### 1. capture

WHY: explain that HippocampusMD starts from complete thoughts, not vague labels. A good title should read naturally inside another sentence.
...
SEE: show the path from raw paragraph to atomic notes to possible graph connections. Mention that `hippocampusmd-reduce` handles this workflow at larger scale.
...
SEE: show a small health table and mention that `hippocampusmd-health` checks broader vault structure while `hippocampusmd-validate` checks note quality in more detail.
...
SEE: show a compact text graph or list of connections and recommend a natural next step: `hippocampusmd-reduce` for source extraction, `hippocampusmd-reflect` for connection discovery, `hippocampusmd-graph` for graph structure, `hippocampusmd-health` for maintenance, `hippocampusmd-pipeline` for end-to-end source processing, or `hippocampusmd-learn` when that future workflow is available.

## Note Quality Expectations

Tutorial-created notes are real vault content only when the user opts in. They should still meet normal HippocampusMD quality expectations:
